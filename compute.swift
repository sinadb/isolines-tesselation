...
class lineTesselation {
    ...
    func updateThreadCountsAndTesselationFactor(){
        switch tesselationMode.rawValue {
        case 2:
            if(controlPointCount == 2){
                threadsLength = (tesselationFactors[0] + 1)
                tesselationFactors[1] = 0
            }
            else{
                threadsLength = (tesselationFactors[0] + 1)*(tesselationFactors[1] + 1)
    ...
    init?(_ device : MTLDevice, _ mode : Mode, with tesselationLevels : [Int], CPCount : Int){
        self.device = device
        ...
        var tempCPCount = CPCount
        computeFunctionConstant.setConstantValue(&tempCPCount, type: .int, index: 1)
    ...
    func updateTesselationLevel(with value : [Int]){
        tesselationFactors = value
        // update the outputbuffer and thread counts
        updateThreadCountsAndTesselationFactor()
    ...
    func initialiseBuffer(with points : [simd_float4]){
        switch tesselationMode.rawValue {
        case 2:
            controlPointBuffer = device.makeBuffer(bytes: points, length: MemoryLayout<simd_float4>.stride*controlPointCount, options: [])
            outputBuffer = device.makeBuffer(length: MemoryLayout<simd_float4>.stride*threadsLength, options: [])
            break
        case 3:
            switch points.count {
...
//            break
//        }
        controlPointBuffer?.contents().copyMemory(from: points, byteCount: MemoryLayout<simd_float4>.stride*points.count)
